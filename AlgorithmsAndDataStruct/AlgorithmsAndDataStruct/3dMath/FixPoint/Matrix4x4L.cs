...
namespace FixPoint
{
    public struct Matrix4x4L
    {
        ...
        {
            string ret = m00.ToString() + ",\t" + m01.ToString() + ",\t" + m02.ToString() + ",\t" + m03.ToString() + "\n";
            ret += m10.ToString() + ",\t" + m11.ToString() + ",\t" + m12.ToString() + ",\t" + m13.ToString() + "\n";
            ret += m20.ToString() + ",\t" + m21.ToString() + ",\t" + m22.ToString() + ",\t" + m23.ToString() + "\n";
            ret += m30.ToString() + ",\t" + m31.ToString() + ",\t" + m32.ToString() + ",\t" + m33.ToString() + "\n";
            return ret;
        }

        public static bool operator ==(Matrix4x4L lhs, Matrix4x4L rhs)
        {
        ...
        public static Matrix4x4L operator *(Matrix4x4L lhs, Matrix4x4L rhs)
        {
            return new Matrix4x4L
            {
        ...
        public static Matrix4x4L identity
        {
            get
            {
                return new Matrix4x4L
        ...
        public FloatL this[int row, int column]
        {
            get
            {
        ...
        public Vector4L GetColumn(int i)
        {
            return new Vector4L(this[0, i], this[1, i], this[2, i], this[3, i]);
        }

        public Vector4L GetRow(int i)
        {
            return new Vector4L(this[i, 0], this[i, 1], this[i, 2], this[i, 3]);
        }
        ...
        public Vector3L MultiplyPoint(Vector3L v)
        {
            Vector3L result;
            ...
            FloatL num = this.m30 * v.x + this.m31 * v.y + this.m32 * v.z + this.m33;
            num = 1f / num;
            result.x *= num;
            result.y *= num;
            result.z *= num;
        ...
        public static Matrix4x4L Scale(Vector3L v)
        {
            return new Matrix4x4L
            {
                m00 = v.x,
        ...
        static FloatL[][] _MatrixCreate(int rows, int cols)
        {
            FloatL[][] result = new FloatL[rows][];
            for (int i = 0; i < rows; ++i)
                result[i] = new FloatL[cols];
            return result;
        }

        static FloatL[][] _MatrixInverse(FloatL[][] matrix)
        {
            // assumes determinant is not 0
            // that is, the matrix does have an inverse
            int n = matrix.Length;
            FloatL[][] result = _MatrixCreate(n, n); // make a copy of matrix
            for (int i = 0; i < n; ++i)
                for (int j = 0; j < n; ++j)
                    result[i][j] = matrix[i][j];

            FloatL[][] lum; // combined lower & upper
            int[] perm;
            int toggle;
            toggle = _MatrixDecompose(matrix, out lum, out perm);

            FloatL[] b = new FloatL[n];
            for (int i = 0; i < n; ++i)
            {
                for (int j = 0; j < n; ++j)
                    if (i == perm[j])
